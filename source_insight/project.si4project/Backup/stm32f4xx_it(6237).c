...
u8 wifi_buf[2048]={0};
int wifi_len=0,rec_finish=0; 

//usart2
void USART2_IRQHandler(void)
{
		u16 rec_data,ch;//接收数据
		//判断是否进入 接收中断
		if(USART_GetITStatus(USART2,USART_IT_RXNE)==SET)//接收数据寄存器中断不为空 （有数据）
		{
			//清空标志位
			USART_ClearITPendingBit(USART2,USART_IT_RXNE);
			//串口接收数据
			rec_data=USART_ReceiveData(USART2);
			wifi_buf[wifi_len++]=rec_data;//保存wifi返回的数据
			//发送数据	接收到的数据通过串口1发出来 
			USART_SendData(USART1,rec_data);
		}
		else if(USART_GetITStatus(USART2,USART_IT_IDLE)==SET)//空闲中断
		{
				if(rec_finish)	
				{
					//一旦进入空闲  表示数据已经发完了  然后我们需要对数据去进行解析
					//在设置完客户端模式以后  测试输出  实际开发的话 建议将该数据的处理  放到外面其他的函数中去进行
					printf("BUF:%s\r\n",wifi_buf);//只是放在这测试  不要写在中断服务函数里面
					wifi_len=0;
					memset(wifi_buf,0,sizeof(wifi_buf));//清空数据
	//			rec_finish=1;//接收结束
					ch=USART2->SR;
					ch=USART2->DR;
				}
